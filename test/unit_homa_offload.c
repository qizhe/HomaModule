...
{
	struct sk_buff *p, *p_prev = NULL;
	list_for_each_entry(p, &self->gro_list, list) {
		if(p_prev != NULL) {
			kfree_skb(p_prev);
			p_prev = NULL;
		}
...
TEST_F(homa_offload, homa_gro_receive__append)
{
	...
	next = list_first_entry(&self->gro_list, typeof(*next), list);
	next = list_next_entry(next, list);

	unit_log_frag_list(next, 1);
...
{
	struct sk_buff *skb, *skb2, *skb3, *next;
	int same_flow;
